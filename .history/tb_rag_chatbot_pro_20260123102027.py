...
class TBExpertChatbot:
    ...
    def generate_answer(self, query: str, relevant_qa: List[Dict[str, Any]]) -> str:
        """Generate professional answer using retrieved context (with optional GPT enhancement)"""

        # Use the best matching answer directly (most accurate for medical info)
...
Please provide a comprehensive, professional answer based on the context above."""
                ...
                return f"Based on TB guidelines: {best_qa['answer']}\n\n*Note: Using direct retrieval mode (API unavailable)*"
        else:
            # Direct retrieval mode (100% accurate, no hallucinations)
            return f"Based on TB medical guidelines: {best_qa['answer']}\n\n*Using direct retrieval from validated knowledge base*"
