...
class QualityPhraser:
    ...
    def variate(self, base_term, context="direct"):
        ...
        variations = []
        for t in templates:
            # Fix grammar: "Please explain describe" -> "Please explain the"
            if "explain" in t.lower() and "describe" in base_term.lower():
                base_term = base_term.replace("Describe", "the").replace("describe", "the")
            
            # Fix clunky: "Standard of care for dose of" -> "Standard dosing for"
            if "Standard of care for" in t and "dose of" in base_term:
                clean_t = "Standard dosing for"
                clean_base = base_term.replace("dose of ", "")
                variations.append(f"{clean_t} {clean_base}")
                continue
...
def generate_v3_dataset():
    ...
    for item in final_pairs:
        ...
        for pre in long_prefixes:
            ...
            if "protocol for protocol for" in new_q.lower(): new_q = new_q.replace("protocol for protocol for", "protocol for")
    ...
    output_file = 'dataset/TB_QA_DATASET_50K_ULTIMATE_V5.json'
